...
func testFS(t *testing.T, fs FileSystem) {
	...
	ctx := context.Background()
	ctx = context.WithValue(ctx, "new-file", true)
...
func TestCopyMoveMemProps(t *testing.T) {
	ctx := context.Background()
	fs := NewMemFS()
...
func TestMemWalkFS(t *testing.T) {
	testCases := []struct {
		desc    string
		buildfs []string
		startAt string
	...
	ctx := context.Background()
	for _, tc := range testCases {
		fs, err := buildMemTestFS(tc.buildfs)
		if err != nil {
...
func buildMemTestFS(buildfs []string) (FileSystem, error) {
